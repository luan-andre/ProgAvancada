...
vector<FiguraGeometrica *> Interpreter::analise(string filename){
    ifstream fin; // Define o arquivo a ser aberto
    ...
    if (!fin.is_open()){ // verifica se houve errro na abertura do arquivo, se não estiver aberto ele notifica e encerra o processo
    ...
    while(fin.good()){ // o laço se mantem enquanto o arquvo nao apresenta erro
        getline(fin, s);// pegar a linha do arquivo e associar a s
        if(fin.good()){ // caso o arquivo nao apresente erro
